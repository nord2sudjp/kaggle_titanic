...
sizes = (0.003,0.005,0.01,0.025, 0.05, 0.1, 0.2, 0.25, 0.3, 0.35)
fold = 10

train_scores = np.zeros((len(sizes), fold))
test_scores = np.zeros((len(sizes), fold))

for fold_, size in enumerate(sizes):
    for j in range(fold):
        X_train,X_test,y_train,y_test = train_test_split(split_before_x,split_before_y,test_size=size,random_state=None)
        ...
        train_scores[fold_,j] = random_forest.score(X_train, y_train)
        test_scores[fold_,j] = random_forest.score(X_test, y_test)
...
X = list(sizes)
plt.plot(X, np.mean(train_scores, axis=1), marker="o")
plt.plot(X, np.mean(test_scores, axis=1), marker="x")
plt.show()