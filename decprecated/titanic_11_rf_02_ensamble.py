...
for train_indexes, test_indexes in kf.split(X_train, y_train):
    ...
    random_forest.fit(X_train.iloc[train_indexes], y_train.iloc[train_indexes])
    
    pred_y = random_forest.predict(X_train.iloc[test_indexes])
    ac = accuracy_score(y_train.iloc[test_indexes], pred_y)
    results.append(ac)
    models.append(random_forest)    

print("model accuracy score:", results)

temp_pred = np.zeros((len(X_test), 5))

for fold_, rf in enumerate(models):
...
kaggle_submission.to_csv("titanic_11_rf_02.csv", index=False)